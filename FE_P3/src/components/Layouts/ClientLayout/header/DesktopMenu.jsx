...
import { pages as rawPages } from "./menuData";
import { useTranslation } from "react-i18next";

const DesktopMenu = () => {
  const { t } = useTranslation("navigation"); // ✅ hook chỉ trong component
  const location = useLocation();
  const [hoveredMenu, setHoveredMenu] = useState(null);

  const isActive = (path) => location.pathname === path;

  // Map pages + children để dịch
  const pages = rawPages.map((p) => ({
    ...p,
    content: p.key ? t(p.key) : p.content, // dịch menu chính
    children: p.children
      ? p.children.map((c) => ({ ...c, content: c.key ? t(c.key) : c.content })) // dịch menu con
      : undefined,
  }));
        ...
        >
          <Button
            ...
            sx={{
              ...
              color:
                isActive(page.to) ||
                (page.children && location.pathname.includes("/dich-vu"))
                  ? "primary.main"
                  : "text.secondary",
              fontWeight:
                isActive(page.to) ||
