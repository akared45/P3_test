...
const ChatAccessControl = ({ appointment }) => {
  ...
  useEffect(() => {
    ...
    const checkTime = () => {
      ...
      if (now < openTime) {
        setStatus('early');
       setTimeLeft(formatDistanceStrict(openTime, now, { locale: vi, addSuffix: true }));
  ...
  if (status === 'early') {
    return (
      <Box sx={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#f5f5f5', p: 2 }}>
        <Paper elevation={3} sx={{ p: 4, textAlign: 'center', maxWidth: 400, borderRadius: 2 }}>
          ...
          <Typography variant="body1" color="text.secondary" mb={1}>
            Phòng tư vấn sẽ mở trong:
          </Typography>
          <Typography variant="h4" color="primary" fontWeight="bold" sx={{ mb: 2 }}>
            {timeLeft}
          </Typography>
          <Alert severity="info" sx={{ justifyContent: 'center' }}>
            Bạn có thể vào phòng sớm 10 phút để chuẩn bị.
          </Alert>
        </Paper>
      </Box>
    );
    ...
    return (
      <Box sx={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#f5f5f5', p: 2 }}>
        <Paper elevation={3} sx={{ p: 4, textAlign: 'center', maxWidth: 400 }}>
          ...
          <Typography variant="h6" gutterBottom>Phiên tư vấn đã kết thúc</Typography>
