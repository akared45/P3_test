...
class BookAppointmentUseCase {
    ...
    async execute(request) {
        ...
        try {
            ...
            this.socketService.sendToUser(doctorIdStr.toString(), 'new_notification', {
                ...doctorNoti, 
                id: doctorNoti.id
            });
            ...
            this.socketService.sendToUser(patientIdStr.toString(), 'new_notification', {
                ...patientNoti,
                id: patientNoti.id
            });

            const timeString = startTime.toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' });
            const dateString = startTime.toLocaleDateString('vi-VN');

            this.emailService.sendAppointmentConfirmation(patient.email, {
                patientName: patient.profile?.fullName || patient.username || "Quý khách", 
