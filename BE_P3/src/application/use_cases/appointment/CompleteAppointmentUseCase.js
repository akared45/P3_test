...
class CompleteAppointmentUseCase {
    ...
    async execute(request) {
        const { userId, appointmentId, doctorNotes, symptoms, prescriptions } = request;

        const appointment = await this.appointmentRepository.findById(appointmentId);
        if (!appointment) throw new Error("Lịch hẹn không tồn tại");

        if (appointment.doctorId.toString() !== userId) {
            throw new Error("Bạn không có quyền kê đơn cho lịch hẹn này");
        }

        const completedAppointment = appointment.complete(doctorNotes, prescriptions, symptoms);
        
        await this.appointmentRepository.save(completedAppointment);

        const patient = await this.userRepository.findById(completedAppointment.patientId);
